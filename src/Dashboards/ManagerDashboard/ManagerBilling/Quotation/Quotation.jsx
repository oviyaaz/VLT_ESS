...
const CreateQuotation = () => {
  ...
  // Query params for partyId, quotationId, and view
  ...
  // If quotationId + view=1 => load from localStorage => show preview
  useEffect(() => {
    if (quotationId && viewParam === "1") {
      const allQuotations = JSON.parse(localStorage.getItem("quotations")) || [];
      const existing = allQuotations.find((q) => String(q.id) === quotationId);
      if (existing) {
        setSavedQuotation(existing);
        setShowQuotationPreview(true);
      }
    }
  }, [quotationId, viewParam]);
  ...
  const handleSaveShippingAddress = () => {
    ...
    if (!editingAddress.id) {
      // If new address, generate a unique ID
      editingAddress.id = Date.now();
      updatedAddresses.push(editingAddress);
    } else {
      // If editing existing address, update the list
    ...
    // Set the updated selected party without using local storage
    setSelectedParty(updatedParty);
    setShippingAddresses(updatedAddresses);
  
    // Close the edit shipping modal
  ...
  const handleDoneAddingItems = async () => {
    try {
      // Filter and map selected items with quantity > 0
      const selectedItems = filteredModalItems
        .filter((_, idx) => parseFloat(modalQuantities[idx] || 0) > 0)
        .map((it, idx) => {
          let gstTax = 0;
          let hsnSacCode = "-"; // Default value for hsn_sac_code
  
          // Handle product type
          if (it.type === "product") {
            gstTax = parseFloat(it.gst_tax) || 0;
            hsnSacCode = it.hsn_sac_code || "-"; // Use hsn_sac_code for products
          }
          // Handle service type
          else if (it.type === "service") {
            gstTax = parseFloat(it.gst_tax_rate) || 0;
            hsnSacCode = it.sac_code || "-"; // Use sac_code for services
          }
  
          console.log("Item Data for HSN/SAC Code Mapping:", it); // Log to check the structure of the item
  
          return {
            ...it,
            qty: parseFloat(modalQuantities[idx]) || 0, // Get quantity from modal input
            discount: 0, // Default discount
            tax: gstTax, // Correct GST tax
            itemName: it.name || "", // Ensure item name is populated
            hsn_sac_code: hsnSacCode, // Correctly set HSN/SAC code
            salesPrice: parseFloat(it.sales_price) || 0, // Default sales price
          };
        });
      ...
      if (selectedItems.length > 0) {
        setQuotationItems((prev) => [...prev, ...selectedItems]); // Add selected items to the invoice
      }
    } catch (error) {
  ...
  const handleRemoveQuotationItem = (idx) => {
    const updated = [...quotationItems];
  ...
  // ------------------------------------------------------------------
  // 6) Mark as Fully Paid for Quotation
  // ------------------------------------------------------------------
  // If you also want to replicate the "mark as fully paid" logic for Quotation:
  const [markAsFullyPaid, setMarkAsFullyPaid] = useState(false);

  // We'll store balanceAmount and receivedAmount in the quotation if fully paid
  const balanceAmount = markAsFullyPaid ? 0 : grandTotal;
  const receivedAmount = markAsFullyPaid ? grandTotal : 0;

  // ------------------------------------------------------------------
  // 7) Save => store quotation => show preview
  // ------------------------------------------------------------------
  const [quotation_number, setQuotationNumber] = useState(1);
  const [quotation_date, setQuotationDate] = useState(new Date().toLocaleDateString());
  const [due_date, setDueDate] = useState("");
  const [paymentTerms, setPaymentTerms] = useState(0);

  const handleSave = async () => {
    try {
      console.log("🛠 Debug: Saving quotation...");
  
      const itemsToMap = Array.isArray(quotationItems) ? quotationItems : [];
      console.log("🛠 Debug: Current quotation Items:", itemsToMap);
      ...
      if (mappedItems.length === 0) {
        throw new Error("❌ No valid items included in the quotation.");
      }
      ...
      // 👉 Decide between POST (create) and PUT (update)
      const method = savedQuotation?.id ? "put" : "post";
      const url = savedQuotation?.id
        ? `${process.env.REACT_APP_BASEAPI}/quotation/quotation-estimate/${savedQuotation.id}/update/`
        : `${process.env.REACT_APP_BASEAPI}/quotation/quotation/`;
  
      const res = await axios[method](url, quotationData);
  
      if (res.status === 200 || res.status === 201) {
        console.log(`✅ Quotation ${method === "put" ? "updated" : "created"} successfully`, res.data);
        setSavedQuotation(res.data);
        setShowQuotationPreview(true);
      } else {
        console.error("❌ Error: Response status is not successful", res.status);
      }
    } catch (error) {
  ...
  // For the "delete quotation" confirmation modal
  const [showDeleteModal, setShowDeleteModal] = useState(false);

  const handleConfirmDelete = async () => {
  try {
    // Make sure quotation ID exists
    if (!savedQuotation?.id) {
      alert("⚠️ Quotation ID not found.");
      return;
    }

    const res = await axios.delete(
      `${process.env.REACT_APP_BASEAPI}/quotation/quotation-estimate/delete/${savedQuotation.id}/`
    );

    if (res.status === 204 || res.status === 200) {
      // Update localStorage
      let all = JSON.parse(localStorage.getItem("quotations")) || [];
      const updated = all.filter((q) => q.id !== savedQuotation.id);
      ...
      navigate("/manager/billing/quotationestimate");
    } else {
      throw new Error("Failed to delete quotation from server.");
    }
  } catch (error) {
    console.error("❌ Error deleting quotation:", error.message);
    alert("⚠️ Something went wrong while deleting the quotation.");
  }
};
  ...
  const handleDownloadPDF = async () => {
    ...
    pdf.save(`quotation_${savedQuotation.quotation_number}.pdf`);
  };

  // ------------------------------------------------------------------
  // 8) If we're in preview mode, show the Quotation preview
  // ------------------------------------------------------------------
  if (showQuotationPreview && savedQuotation) {
    const isPaid = savedQuotation.balanceAmount === 0;

    return (
      <>
        {/* TOP BAR => Quotation #, Paid/Unpaid, Download, Print */}
        <div className="p-4 flex items-center justify-between border-b bg-white ">
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-semibold">
              Quotation #{savedQuotation.quotation_number}
            </h2>
            <span
        ...
        <div className="p-4 flex items-center justify-end space-x-2 border-b bg-white ">
          <button
            onClick={() => {
              // Reopen the form with saved quotation data
              setQuotationItems(savedQuotation.items);
              setQuotationNumber(savedQuotation.quotation_number);
              setQuotationDate(savedQuotation.quotation_date);
              setDueDate(savedQuotation.due_date);
              setPaymentTerms(savedQuotation.paymentTerms || 0);
              setSelectedParty(savedQuotation.party);
              setSubtotal(savedQuotation.subtotal);
              setTaxTotal(savedQuotation.taxTotal);
              setDiscountTotal(savedQuotation.discountTotal);
              setGrandTotal(savedQuotation.grandTotal);

              // Also restore isFullyPaid and check box
              setMarkAsFullyPaid(savedQuotation.isFullyPaid);
        ...
        >
          ...
          <div className="flex justify-between border-b pb-2 mb-2">
            <div className="font-bold uppercase text-sm">QUOTATION</div>
 ...
 {/* Display address */}
            ...
            <div className="text-right">
              <div>Quotation No: {savedQuotation.quotation_number}</div>
              <div>Quotation Date: {savedQuotation.quotation_date}</div>
              <div>Due Date: {savedQuotation.due_date || "--/--/----"}</div>
            </div>
          </div>
          ...
          <table className="w-full text-sm border border-gray-300 mb-2">
            ...
            <tbody>
 {savedQuotation?.quotation_estimate_item?.map((it, index) => {
  const name = it.product_item ? `Product ${it.item_name}` : `Service ${it.service_name || 'N/A'}`;
...
<div className="flex justify-end">
    ...
    <tbody>
      <tr className="border-b">
        ...
        <td className="px-2 py-1 border-r">
          {savedQuotation.quotation_estimate_item?.length ?? 0}
        </td>
        <td className="px-2 py-1 text-right">
          ₹ {parseFloat(savedQuotation.quotation_estimate?.total || 0).toFixed(2)}
        </td>
      </tr>
      <tr>
        <td className="px-2 py-1 font-semibold border-r">RECEIVED AMOUNT</td>
        <td colSpan={2} className="px-2 py-1 text-right">
          ₹ {parseFloat(savedQuotation.receivedAmount || 0).toFixed(2)}
        </td>
      </tr>
    </tbody>
  </table>
  ...
  // ------------------------------------------------------------------
  // 9) Otherwise, show normal create-quotation form
  // ------------------------------------------------------------------
  return (
    <>
      <div className="h-screen flex flex-col overflow-hidden bg-gray-100">
        ...
        {/* Main Create-Quotation Form */}
        <div className="flex-1 overflow-y-auto p-4">
          {/* TOP ROW: BILL TO, SHIP TO, QUOTATION DETAILS */}
    ...
    const lineTotal = subtotal + taxAmount - discountAmount;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            ...
            <div className="space-y-4 text-sm">
              ...
              {/* Mark as Fully Paid for Quotation */}
              ...
              <div className="flex justify-between items-center">
                ...
                >
                  ₹ {markAsFullyPaid ? 0 : grandTotal.toFixed(2)}
                </span>
              </div>
