...
const ItemDetailsPage = () => {
      ...
      <div className="w-64 bg-white border-r p-4 space-y-4">
        ...
        <div className="overflow-y-auto max-h-[70vh] space-y-2">
          {filtered.map((it) => (
            <div
              key={it.id}
              onClick={() => navigate(`/manager/billing/itemdetails/${it.id}`)}
  ...
  onClick={() => {
    ...
    navigate("/manager/billing/itemspage"); // go back to ItemsPage
  }}
...
>
  Edit
            ...
            <button onClick={() => navigate("/manager/billing/itemspage")} className="px-3 py-1 bg-gray-300 text-sm rounded">Close</button>
          </div>
        </div>
