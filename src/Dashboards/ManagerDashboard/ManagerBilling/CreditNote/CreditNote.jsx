...
const CreateCreditNote = () => {
  ...
  // 2) If preview mode, store + load from localStorage("creditNotes")
 ...
 useEffect(() => {
  ...
  // If the user wants to track credit limit, we do the same logic as invoice
  useEffect(() => {
...
console.log("🔍 Filtered Items:", filteredModalItems);
  ...
  const [savedNote, setSavedNote] = useState(null); // we already have savedCreditNote, but let's keep consistent
  const [showInvoicePreview, setShowInvoicePreview] = useState(false); // or we can reuse the same states

  const [credit_note_number, setCreditNoteNumber] = useState(1);
  const [credit_note_date, setCreditNoteDate] = useState(new Date().toLocaleDateString());
  const [due_date, setDueDate] = useState("");

  const handleSave = async () => {
    try {
      console.log("🛠 Debug: Saving Credit Note...");
      ...
      if (mappedItems.length === 0) {
        throw new Error("❌ No valid items included in the credit note.");
      }
      ...
      console.log("📤 Sending Credit Note Data:", creditNoteData);
  
      // ✅ Choose method and URL dynamically
      const method = savedCreditNote?.id ? "put" : "post";
      const url = savedCreditNote?.id
        ? `${process.env.REACT_APP_BASEAPI}/credit-note/${savedCreditNote.id}/update/`
        : `${process.env.REACT_APP_BASEAPI}/credit/create/`;
  
      const res = await axios[method](url, creditNoteData);
  
      if (res.status === 200 || res.status === 201) {
        console.log(`✅ Credit Note ${method === "put" ? "updated" : "created"} successfully`, res.data);
 ...
 const handleConfirmDelete = async () => {
  try {
    if (!savedCreditNote?.id) {
      alert("⚠️ Credit Note ID not found.");
      return;
    }

    // Use the ID in the URL for deletion
    const url = `${process.env.REACT_APP_BASEAPI}/credit/credit-notes/${savedCreditNote.id}/delete/`;
    const res = await axios.delete(url);

    if (res.status === 204 || res.status === 200) {
      // 🧹 Remove from localStorage
      let all = JSON.parse(localStorage.getItem("creditNotes")) || [];
      const updated = all.filter(cn => cn.id !== savedCreditNote.id);
    ...
    alert("Something went wrong while deleting the Credit Note.");
  }
};
  ...
  const handleDownloadPDF = async () => {
    ...
    pdf.save(`credit_note_${savedCreditNote.credit_note_number}.pdf`);
  };

  // 9) If showCreditNotePreview && savedCreditNote, show preview
  if (showCreditNotePreview && savedCreditNote) {
    const isPaid = savedCreditNote.balanceAmount === 0;

    return (
      <>
        {/* TOP BAR => Credit Note #, Paid/Unpaid, Download, Print, etc. */}
        <div className="p-4 flex items-center justify-between border-b bg-white ">
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-semibold">
              Credit Note #{savedCreditNote.credit_note_number}
            </h2>
            <span
        ...
        <div className="p-4 flex items-center justify-end space-x-2 border-b bg-white ">
          <button
            onClick={() => {
              ...
              setCreditNoteNumber(savedCreditNote.credit_note_number);
              setCreditNoteDate(savedCreditNote.credit_note_date);
              setDueDate(savedCreditNote.due_date);
        ...
        >
          ...
          <div className="flex justify-between border-b pb-2 mb-2">
            <div className="font-bold uppercase text-sm">CREDIT NOTE</div>
 ...
 {/* Display address */}
            ...
            <div className="text-right">
              <div>Credit Note No: {savedCreditNote.credit_note_number}</div>
              <div>Credit Note Date: {savedCreditNote.credit_note_date}</div>
              <div>Due Date: {savedCreditNote.due_date || "--/--/----"}</div>
            </div>
          </div>
          ...
          <table className="w-full text-sm border border-gray-300 mb-2">
           ...
           <tbody>
  {(savedCreditNote?.credit_note_items ?? []).length === 0 ? (
    <tr>
    ...
    (savedCreditNote?.credit_note_items ?? []).map((it, index) => {
    ...
    <tbody>
      <tr className="border-b">
        ...
        <td className="px-2 py-1 border-r">
          {(savedCreditNote?.credit_note_items ?? []).length}
        </td>
        <td className="px-2 py-1 text-right">
          ₹ {parseFloat(savedCreditNote?.credit_note_total?.total || 0).toFixed(2)}
        </td>
      </tr>
      <tr>
  ...
  // 10) Otherwise, show normal create-credit-note form
  return (
    <>
      <div className="h-screen flex flex-col overflow-hidden bg-gray-100">
        ...
        {/* Main Create-Credit-Note Form */}
        <div className="flex-1 overflow-y-auto p-4">
          {/* TOP ROW: BILL TO, SHIP TO, CREDIT NOTE DETAILS */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-white border rounded p-4 space-y-3">
              <div className="grid grid-cols-2 gap-2 items-center">
                <label className="text-sm text-gray-600">Credit Note No</label>
                <input
                  type="text"
                  placeholder="ex: 1"
