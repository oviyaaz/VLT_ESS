...
const CreateDeliveryChallan = () => {
  ...
  // Query params for partyId, creditNoteId, and view
  ...
  // ------------------- 2) If preview mode, load from localStorage("creditNotes") -------------------
  ...
  // If a creditNoteId + view=1 is provided, load from "creditNotes"
  useEffect(() => {
    if (creditNoteId && viewParam === "1") {
      const allNotes = JSON.parse(localStorage.getItem("creditNotes")) || [];
      const existing = allNotes.find((cn) => String(cn.id) === creditNoteId);
      if (existing) {
        setSavedCreditNote(existing);
        setShowCreditNotePreview(true);
      }
    }
  }, [creditNoteId, viewParam]);
  ...
  // ------------------- 4) Credit Note Items & Calculations -------------------
  ...
  // 5) Load itemCategories from localStorage
...
const handleCreateNewItem = () => {
  navigate("/manager/billing/items?createNew=1");
};
...
const handleDoneAddingItems = () => {
  ...
  const selectedItems = filteredModalItems
    ...
    .map((it, idx) => ({
      ...it,
      qty: parseFloat(modalQuantities[idx]), // Set the quantity based on modal quantities
      discount: 0, // Default discount is 0
      tax: 0,      // Default tax is 0
      itemName: it.name || "", // Ensure itemName is populated
      hsnCode: it.hsn_code || "-",  // Fallback to "-" if hsnCode is missing
      salesPrice: it.sales_price || 0, // Default to 0 if salesPrice is missing
    }));

  console.log("✅ Selected Items:", selectedItems);

  // Add selected items to the invoice if any are selected
  if (selectedItems.length > 0) {
    setCreditNoteItems((prev) => [...prev, ...selectedItems]); // Add selected items to the invoice
  }

  handleCloseAddItemsModal(); // Close the modal after adding items
};
  ...
  const handleRemoveCreditNoteItem = (idx) => {
    const updated = [...creditNoteItems];
    updated.splice(idx, 1);
    setCreditNoteItems(updated);
  };

  // 7) Save => store credit note => show preview
  const [savedNote, setSavedNote] = useState(null);
  const [showInvoicePreview, setShowInvoicePreview] = useState(false);

  // "Challan No" and "Challan Date"
  const [delivery_challan_number, setdelivery_challan_number] = useState(1);
  const [delivery_challan_date, setdelivery_challan_date] = useState(new Date().toLocaleDateString());

  const handleSave = async () => {
    try {
      console.log("🛠 Debug: Saving Delivery Challan...");
  
      // Ensure creditNoteItems is correctly populated before mapping
      console.log("🛠 Debug: Current Challan Items:", creditNoteItems);
  
      const mappedItems = (Array.isArray(creditNoteItems) ? creditNoteItems : [])
        .filter(item => item?.id)
        .map(item => ({
      ...
      if (mappedItems.length === 0) {
        throw new Error("❌ No valid items included in the delivery challan.");
      }
      ...
      console.log("📤 Sending Delivery Challan Data:", invoiceData);
  
      // ✅ Choose method and URL dynamically
      const method = savedCreditNote?.id ? "put" : "post";
      const url = savedCreditNote?.id
        ? `${process.env.REACT_APP_BASEAPI}/delivery-challan/${savedCreditNote.id}/update/`
        : `${process.env.REACT_APP_BASEAPI}/deliverych/create/`;
  
      const res = await axios[method](url, invoiceData);
  
      if (res.status === 200 || res.status === 201) {
        console.log(`✅ Delivery Challan ${method === "put" ? "updated" : "created"} successfully`, res.data);
        setSavedCreditNote(res.data);
        setShowCreditNotePreview(true);
      } else {
  ...
  try {
    if (!savedCreditNote?.id) {
      alert("⚠️ Delivery Challan ID not found.");
      return;
    }

    // New backend endpoint using the delivery challan ID
    const url = `${process.env.REACT_APP_BASEAPI}/deliverych/delivery-challan/${savedCreditNote.id}/delete/`;
    const res = await axios.delete(url);

    if (res.status === 204 || res.status === 200) {
      // 🧹 Clean up localStorage
      let allCreditNotes = JSON.parse(localStorage.getItem("creditNotes")) || [];
      const updatedCreditNotes = allCreditNotes.filter((cn) => cn.id !== savedCreditNote.id);
      ...
      const updatedChallans = allChallans.filter((ch) => ch.id !== savedCreditNote.id);
      localStorage.setItem("deliveryChallans", JSON.stringify(updatedChallans));

      // 🧼 Reset UI State
      setShowCreditNotePreview(false);
    ...
    console.error("❌ Error deleting Delivery Challan:", error.message);
    alert("Something went wrong while deleting the Delivery Challan.");
  }
};
  ...
  const handleDownloadPDF = async () => {
    ...
    pdf.save(`credit_note_${savedCreditNote.delivery_challan_number}.pdf`);
  };

  // 10) Convert to Invoice logic
  const handleConvertToInvoice = () => {
    savedCreditNote.isFullyPaid = false; // or keep it false if you want "Unpaid"
    savedCreditNote.balanceAmount = savedCreditNote.grandTotal;

    // Update the "creditNotes"
    let all = JSON.parse(localStorage.getItem("creditNotes")) || [];
    const idx = all.findIndex((cn) => cn.id === savedCreditNote.id);
    if (idx >= 0) {
      all[idx] = { ...savedCreditNote };
      localStorage.setItem("creditNotes", JSON.stringify(all));
    }

    // Also add to "invoices" if you want it to appear in your SalesInvoice page
    let allInvoices = JSON.parse(localStorage.getItem("invoices")) || [];
    // For an invoice, we might rename creditNoteNumber => invoiceNumber, creditNoteDate => invoiceDate
    const convertedInvoice = {
      ...savedCreditNote,
      invoiceNumber: savedCreditNote.delivery_challan_number,
      invoiceDate: savedCreditNote.delivery_challan_date,
    };
    allInvoices.push(convertedInvoice);
    localStorage.setItem("invoices", JSON.stringify(allInvoices));

    // If you also want to update "deliveryChallans" to show it as "Closed"
    let allChallans = JSON.parse(localStorage.getItem("deliveryChallans")) || [];
    const cIdx = allChallans.findIndex((ch) => ch.id === savedCreditNote.id);
    if (cIdx >= 0) {
      allChallans[cIdx].status = "Closed";
      localStorage.setItem("deliveryChallans", JSON.stringify(allChallans));
    }

    alert("Converted to Invoice! Now it appears in your Sales Invoices as 'Unpaid'.");
  };

  // 11) If showCreditNotePreview && savedCreditNote, show preview
  if (showCreditNotePreview && savedCreditNote) {
    const isClosed = savedCreditNote.isFullyPaid;
      ...
      <>
        ...
        <div className="p-4 flex items-center justify-between border-b bg-white ">
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-semibold">
              Challan #{savedCreditNote.delivery_challan_number}
            </h2>
            <span
        ...
        <div className="p-4 flex items-center justify-end space-x-2 border-b bg-white ">
          {!isClosed && (
            <button
              ...
              className="px-3 py-1 border rounded hover:bg-gray-100 text-sm text-blue-600"
            >
              Convert to Invoice
            </button>
          )}
          <button
            onClick={() => {
              setCreditNoteItems(savedCreditNote.items);
              setdelivery_challan_number(savedCreditNote.delivery_challan_number);
              setdelivery_challan_date(savedCreditNote.delivery_challan_date);
              setSelectedParty(savedCreditNote.party);
              setSubtotal(savedCreditNote.subtotal);
              setTaxTotal(savedCreditNote.taxTotal);
              setDiscountTotal(savedCreditNote.discountTotal);
              setGrandTotal(savedCreditNote.grandTotal);
              setMarkAsFullyPaid(savedCreditNote.isFullyPaid);
        ...
        >
          ...
          <div className="flex justify-between border-b pb-2 mb-2">
            <div className="font-bold uppercase text-sm">DELIVERY CHALLAN</div>
 ...
 {/* Display address */}
            ...
            <div className="text-right">
              <div>Challan No: {savedCreditNote.delivery_challan_number}</div>
              <div>Challan Date: {savedCreditNote.delivery_challan_date}</div>
            </div>
          </div>
...
<table className="w-full text-sm border border-gray-300 mb-2">
  ...
  <tbody>
    {savedCreditNote?.delivery_challan_items?.length > 0 ? (
      savedCreditNote.delivery_challan_items.map((it, index) => {
        const name = it.product_item ? it.item_name : "Service N/A";  // Adjust according to your data
        ...
        return (
          <tr key={index} className="border-b">
            ...
            <td className="p-2 border-r text-center">{qty}</td>
            <td className="p-2 border-r text-center">{rate.toFixed(2)}</td>
...
<div className="flex justify-end">
  <table className="text-sm border">
    <tbody>
      <tr className="border-b">
        <td className="px-4 py-1 font-semibold border-r">TOTAL</td>
        <td className="px-4 py-1 border-r">
          {savedCreditNote?.delivery_challan_items?.length ?? 0}
        </td>
        <td className="px-4 py-1 text-right">
          ₹ {parseFloat(savedCreditNote?.delivery_challan_total?.total || 0).toFixed(2)}
        </td>
      </tr>
      <tr>
        <td className="px-4 py-1 font-semibold border-r">
          RECEIVED AMOUNT
        </td>
        <td colSpan={2} className="px-4 py-1 text-right">
          {savedCreditNote?.status === "Paid" ? (
            <>₹ {parseFloat(savedCreditNote?.delivery_challan_total?.total || 0).toFixed(2)}</>
          ) : (
            <>₹ 0</>
          )}
  ...
  // 12) Normal Create-Delivery-Challan form
  return (
    <>
      <div className="h-screen flex flex-col overflow-hidden bg-gray-100">
        ...
        {/* Main Create-Delivery-Challan Form */}
        <div className="flex-1 overflow-y-auto p-4">
          {/* TOP ROW: Bill To, Ship To, and now Challan No & Challan Date */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-white border rounded p-4 space-y-2">
              ...
              <p className="text-sm text-gray-600">
                Phone Number:{" "}
                {selectedParty? selectedParty.mobile_number : "[xxxxxxx]"}
              </p>
              {showPartyDropdown && (
            ...
            <div className="bg-white border rounded p-4 space-y-3">
              <div className="grid grid-cols-2 gap-2 items-center">
                <label className="text-sm text-gray-600">Challan No</label>
                <input
                  type="text"
                  placeholder="ex: 1"
                  value={delivery_challan_number}
                  onChange={(e) => setdelivery_challan_number(e.target.value)}
                  className="border rounded p-1 text-sm"
                />
              </div>
              <div className="grid grid-cols-2 gap-2 items-center">
                <label className="text-sm text-gray-600">Challan Date</label>
                <input
                  type="date"
                  value={delivery_challan_date}
                  onChange={(e) => setdelivery_challan_date(e.target.value)}
                  className="border rounded p-1 text-sm"
                />
              </div>
            </div>
            ...
            {creditNoteItems.length === 0 ? (
              ...
              creditNoteItems.map((item, idx) => {
                ...
                if (item.type === "product") {
                  ...
                  gstTaxRate = parseFloat(item.gst_tax || 0);
                  lineTotal = parseFloat(item.final_total || 0);
                }
                // Handle service type
                  ...
                  price = item.sales_price || "0";
                  gstTaxRate = parseFloat(item.gst_tax_rate || 0);
                  lineTotal = parseFloat(item.final_total || 0);
                }
            
                const qty = parseFloat(item.qty || 0); // Quantity
                const discount = parseFloat(item.discount || 0); // Discount
                const lineSubtotal = qty * price; // Line subtotal
                const lineDiscount = (lineSubtotal * discount) / 100; // Line discount
                const lineTax = (lineSubtotal * gstTaxRate) / 100; // Line tax
                const totalAmount = lineSubtotal + lineTax - lineDiscount; // Total amount after discount and tax
                ...
                return (
                  <div key={idx} className="grid grid-cols-8 gap-2 text-sm items-center border-b py-2">
                    ...
                    <div className="text-center">
                      <select
                        className="border rounded p-1 text-sm"
                        value={taxOptions.find((o) => o.value === gstTaxRate)?.label || "None"}
                    ...
                    <div className="col-span-8 flex justify-end">
                      <button
                        onClick={() => handleRemoveCreditNoteItem(idx)}
        ...
        <tr key={index} className="border-b">
          ...
          <td className="p-2">
            {parseFloat(it.current_stock || 0) <= 0 ? (
              <span className="text-red-500">{stock} (Insufficient Stock)</span>
            ) : (
              <>{stock}</>
            )}
          </td>
          <td className="p-2">
            <input
              ...
              onChange={(e) => handleChangeModalQuantity(index, e.target.value)}
            />
          </td>
        </tr>
      );
