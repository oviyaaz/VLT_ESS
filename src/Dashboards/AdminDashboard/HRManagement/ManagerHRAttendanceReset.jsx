...
const fetchResetRequests = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/admin/hr-reset-requests/`);
  return data.reset_requests || [];
};

const ManagerHRAttendanceReset = () => {
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      await axios.post(`${apiBaseUrl}/admin/hr-approve-and-reset-checkout-time/${requestId}/`);
  ...
  const handleReject = async (requestId) => {
    setIsProcessing((prev) => ({ ...prev, [requestId]: true }));
    try {
      await axios.post(`${apiBaseUrl}/admin/hr-reject-reset-request/${requestId}/`);
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              <div>
                ...
                <p className="text-gray-500 text-sm">Manage HR attendance reset requests</p>
              </div>
              <Button
                  ...
                  {resetRequests.length === 0 ? (
                    ...
                    resetRequests.map((request, index) => (
                      <TableRow key={request.id} className="text-base hover:bg-gray-50">
                        ...
                        <TableCell>{request.hr_id}</TableCell>
                        <TableCell>{request.hr_name}</TableCell>
                        ...
                        <TableCell className="flex gap-2">
                          <Button
                            ...
                            aria-label={`Approve request for ${request.hr_name}`}
                          ...
                          <Button
                            ...
                            disabled={request.status !== "Pending" || isProcessing[request.id]}
                            aria-label={`Reject request for ${request.hr_name}`}
...
export default ManagerHRAttendanceReset;