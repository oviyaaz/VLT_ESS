...
const ManagerHRChart = () => {
  ...
  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/api/hr_list/`)
      .then((response) => {
        setHrs(response.data);
      })
      .catch((error) => console.error("Error fetching hrs:", error));
  }, []);
  ...
  const fetchWeeklyChart = (offset) => {
    ...
    axios
      .post(`${apiBaseUrl}/api/admin-hr-weekly-chart/`, {
        hr_id: selectedHr,
        week_offset: offset,
      })
      .then((response) => {
  ...
  const fetchMonthlyChart = (offset) => {
    if (!selectedHr) return;
    axios
      .post(`${apiBaseUrl}/api/admin-hr-monthly-chart/`, {
        hr_id: selectedHr,
        month_offset: offset,
      })
      .then((response) => {
  ...
  return (
    <div className="employeeChart p-4 h-full flex flex-col w-full gap-4">
      ...
      <div className="flex w-full justify-between items-center">
        <h3 className="text-h3">HR Attendance Chart</h3>
