...
const fetchAttendanceRecords = async (hrId) => {
  if (!hrId || hrId === "all") {
    const { data } = await axios.get(`${apiBaseUrl}/admin/hr-attendance-history/`);
    return data.all_records.map((record, index) => ({
      id: `${record.hr_id || 'all'}-${record.date}-${index}`,
      ...record,
    })) || [];
  }
  const { data } = await axios.get(`${apiBaseUrl}/admin/hr-attendance-history/`, {
...
const AttendanceCard = ({ record, index }) => (
  <Card className="shadow-sm">
    ...
    <CardContent className="space-y-2">
      <p><strong>HR:</strong> {record.hr_name}</p>
...
const HrAttendanceRecords = ({ hrId }) => {
  ...
  const { data: attendanceRecords = [], isFetching, isError } = useQuery({
    queryKey: ["hr_attendance", hrId],
    queryFn: () => fetchAttendanceRecords(hrId),
    placeholderData: [],
    staleTime: 5000,
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (record) =>
          record.hr_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <div className="bg-white rounded-lg shadow-sm mb-4">
            <div className="flex flex-col md:flex-row justify-between p-4">
              <div>
                ...
                <p className="text-gray-500 text-sm">Overview of HR attendance records</p>
              </div>
            </div>
...
const ManagerHRAttendance = () => {
  const queryClient = useQueryClient();
  const [selectedHrId, setSelectedHrId] = useState("all");
  ...
  const handleRefresh = () => {
    queryClient.invalidateQueries(["hrs"]);
    queryClient.invalidateQueries(["hr_attendance"]);
  };
  ...
  return (
    <div className="p-2 sm:p-4 min-h-screen">
      {isFetchingHrs ? (
        ...
        <Alert variant="destructive" className="text-center my-4">
          Failed to load HR list. Please try again.
        </Alert>
      ) : (
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">HR Attendance</h5>
              <p className="text-gray-500 text-sm">Select an HR to view attendance records</p>
            </div>
            <div className="mt-2 md:mt-0 flex gap-3">
              <Select
                value={selectedHrId}
                onValueChange={setSelectedHrId}
              >
          ...
          <HrAttendanceRecords hrId={selectedHrId} />
        </div>
      )}
    </div>
  );
...
export default ManagerHRAttendance;