...
const UpdateManagerHRSalary = ({ open, setOpen, salaryId, HrList, fetchSalaryList }) => {
  ...
  const validateForm = () => {
    ...
    // Validate HR Selection
    if (!SalaryData.user_id) {
      newErrors.user_id = "Please select an HR personnel.";
      valid = false;
    }
  ...
  const HandleUpdateSalary = async (e) => {
    ...
    try {
      await axios.put(`${apiBaseUrl}/update-hr-salary/${salaryId.id}/`, SalaryData, {
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Update HR Salary</DialogTitle>
        </DialogHeader>
        <form className="space-y-6" onSubmit={HandleUpdateSalary}>
          <div className="space-y-4">
            <div>
              <label className="text-sm text-gray-600">HR Name</label>
              <select
                id="user_id"
...
export default UpdateManagerHRSalary;