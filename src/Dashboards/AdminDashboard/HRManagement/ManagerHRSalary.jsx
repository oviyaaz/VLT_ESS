...
import AddManagerHRSalary from "./AddManagerHRSalary";
import UpdateManagerHRSalary from "./UpdateManagerHRSalary";
...
const fetchSalaryList = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/hr-salary/history/`);
  return data || [];
};
...
const ManagerHRSalary = () => {
  ...
  const { data: hrList = [], isFetching: isFetchingHrs, isError: isErrorHrs } = useQuery({
    queryKey: ["hrs"],
    queryFn: fetchHrList,
    placeholderData: [],
    refetchOnWindowFocus: false,
  ...
  const handleDelete = async (salary) => {
    if (window.confirm(`Are you sure you want to delete Salary ID ${salary.id}?`)) {
      try {
        await axios.delete(`${apiBaseUrl}/delete-hr-salary/${salary.id}/`);
        toast.success(`Salary ID ${salary.id} deleted successfully.`);
        queryClient.invalidateQueries(["hrSalaries"]);
      } catch (error) {
  ...
  return (
    <div className="p-2 sm:p-4 min-h-screen">
      {(isFetchingSalaries || isFetchingHrs) ? (
        <SkeletonLoading />
      ) : (isErrorSalaries || isErrorHrs) ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">HR Salary Records</h5>
              <p className="text-gray-500 text-sm">Manage salary details for HR personnel</p>
            </div>
            <div className="mt-2 md:mt-0 flex gap-3">
              <Button
                onClick={() => queryClient.invalidateQueries(["hrSalaries"])}
                  ...
                  salaryList.map((salary, index) => (
                    <TableRow key={salary.id}>
                      ...
                      <TableCell className="text-base">
                        {hrList.find((h) => h.id === salary.user_id)?.hr_name || salary.user_id}
                      </TableCell>
          ...
          {addSalaryPopup && (
            <AddManagerHRSalary
              ...
              HrList={hrList}
              fetchSalaryList={() => queryClient.invalidateQueries(["hrSalaries"])}
            />
          )}
          {updateSalaryPopup && selectedSalary && (
            <UpdateManagerHRSalary
