...
const fetchAllAttendanceRecords = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/user/all-user-attendance-history/`);
  return data.all_records?.map((record, index) => ({
    id: `${record.user_id || 'all'}-${record.date}-${index}`,
    ...record,
  })) || [];
};
...
const UserAttendanceRecords = ({ allRecords, selectedUserId }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    // Group records by user_id and date, keeping the earliest check-in
    ...
    if (selectedUserId !== "all") {
      filtered = filtered.filter(
        (record) => record.user_id === selectedUserId
      );
    }
    ...
    return filtered;
  }, [searchTerm, sortConfig, allRecords, selectedUserId]);
          ...
          <div className="bg-white rounded-lg shadow-sm mb-4">
            <div className="flex flex-col md:flex-row justify-between p-4">
              <div>
                ...
                <p className="text-gray-500 text-sm">Overview of user attendance records</p>
              </div>
            </div>
...
const UserAttendance = () => {
  ...
  const { data: users = [], isFetching: isFetchingUsers, isError: isErrorUsers } = useQuery({
    queryKey: ["users"],
    queryFn: fetchUsers,
    placeholderData: [],
    staleTime: 5000,
  ...
  const handleRefresh = () => {
    ...
    queryClient.invalidateQueries(["all_users_attendance"]);
  };

  return (
    <div className="p-2 sm:p-4 min-h-screen">
      {(isFetchingUsers || isFetchingRecords) ? (
        <SkeletonLoading />
      ) : isErrorUsers || isErrorRecords ? (
        <Alert variant="destructive" className="text-center my-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">Users Attendance</h5>
              <p className="text-gray-500 text-sm">Select an user to view attendance records</p>
            </div>
            <div className="mt-2 md:mt-0 flex gap-3">
              <Select
          ...
          <UserAttendanceRecords allRecords={allRecords} selectedUserId={selectedUserId} />
        </div>
      )}
    </div>
  );
