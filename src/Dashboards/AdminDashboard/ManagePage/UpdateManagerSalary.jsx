...
const UpdateManagerSalary = ({ open, setOpen, salaryId, ManagerList, fetchSalaryList }) => {
  ...
  const validateForm = () => {
    ...
    if (!SalaryData.user_id) {
      newErrors.user_id = "Please select a manager.";
      valid = false;
    }
  ...
  const HandleUpdateSalary = async (e) => {
    ...
    try {
      await axios.put(`${apiBaseUrl}/update-manager-salary/${salaryId.id}/`, SalaryData, {
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Update Manager Salary</DialogTitle>
        </DialogHeader>
