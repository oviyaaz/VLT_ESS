...
const ManagerLateLoginReason = () => {
  ...
  const fetchLateLoginReasons = async (retryCount = 3) => {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}/admin_manager_late_login_reasons/`,
      ...
      console.error("Error fetching late login reasons for managers:", error);
      if (retryCount > 0) {
        console.log(`Retrying fetchLateLoginReasons... (${retryCount} attempts left)`);
        setTimeout(() => fetchLateLoginReasons(retryCount - 1), 1000);
      } else {
        toast.error("Failed to fetch late login reasons for managers.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleApprove = async (reasonId) => {
    try {
      const response = await axios.post(
        `${apiBaseUrl}/approve_manager_late_login_reason/${reasonId}/`,
        {},
        { withCredentials: true }
      );
      ...
      console.error("Error approving manager reason:", error);
  ...
  const handleReject = async (reasonId) => {
    if (!window.confirm("Rejecting this reason will keep the leave approved and may deduct from the manager's leave balance. Continue?")) {
      return;
    }

    try {
      const response = await axios.post(
        `${apiBaseUrl}/reject_manager_late_login_reason/${reasonId}/`,
        {},
        { withCredentials: true }
      );
      ...
      console.error("Error rejecting manager reason:", error);
  ...
  return (
    <div className="h-full p-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-semibold">Manager Late Login Reason Submission</h3>
        <button
            ...
            {loading ? (
              ...
              lateLoginReasons.map((reason, index) => (
                <TableRow key={reason.id}>
                  ...
                  <TableCell>{reason.manager_name}</TableCell>
              ...
              <TableRow>
                <TableCell colSpan="7" className="text-center">
                  No late login reasons found for managers.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
...
export default ManagerLateLoginReason;