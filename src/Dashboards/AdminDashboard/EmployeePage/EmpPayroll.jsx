...
const fetchPayrollHistory = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/payroll-history/`);
  return data || [];
};
...
const EmpPayroll = () => {
  ...
  const handleGeneratePayslip = async () => {
    ...
    try {
      const response = await axios.post(`${apiBaseUrl}/process-payroll/`, {
  ...
  return (
    <div className="p-2 sm:p-4 min-h-screen">
      {(isFetchingEmployees || isFetchingPayroll) ? (
        <SkeletonLoading />
      ) : (isErrorEmployees || isErrorPayroll) ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">Employee Payroll</h5>
              <p className="text-gray-500 text-sm">Generate and manage payroll records for employees</p>
            </div>
