...
const EmployeeLateLoginReason = () => {
  ...
  const fetchLateLoginReasons = async (retryCount = 3) => {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}/admin_employee_late_login_reasons/`,
  ...
  const handleApprove = async (reasonId) => {
    try {
      const response = await axios.post(
        `${apiBaseUrl}/approve_employee_late_login_reason/${reasonId}/`,
        {},
        { withCredentials: true }
      );
  ...
  const handleReject = async (reasonId) => {
    ...
    if (!window.confirm("Rejecting this reason will keep the leave approved and may deduct from the employee's leave balance. Continue?")) {
      return;
    }

    try {
      const response = await axios.post(
        `${apiBaseUrl}/reject_employee_late_login_reason/${reasonId}/`,
        {},
        { withCredentials: true }
      );
  ...
  return (
    <div className="h-full p-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-semibold">Employee Late Login Reason Submission</h3>
        <button
            ...
            {loading ? (
              ...
              lateLoginReasons.map((reason, index) => (
                <TableRow key={reason.id}>
                  ...
                  <TableCell>{reason.employee_name}</TableCell>
...
export default EmployeeLateLoginReason;